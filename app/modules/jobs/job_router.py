...
async def create_job(
    ...
    """Ajoute une offre d'emploi manuellement de manière asynchrone."""
    job = await JobService.create(db, data)
    return created(
...
async def list_jobs(
    ...
    """Liste paginée des offres avec filtres combinables (Asynchrone)."""
    page = await JobService.get_all(
        db=db,
        params=params,
        keyword=keyword,
        location=location,
...
async def get_job(
    ...
    """Retourne le détail complet d'une offre de manière asynchrone. HTTP 404 si introuvable."""
...
async def update_job(
    ...
    """Mise à jour partielle asynchrone. Seuls les champs envoyés sont modifiés."""
    job = await JobService.update(db, job_id, data)
    return ok(
        data=JobResponse.model_validate(job).model_dump(mode="json"),
...
async def delete_job(
    ...
    """Suppression définitive asynchrone. HTTP 404 si introuvable."""
    result = await JobService.delete(db, job_id)
