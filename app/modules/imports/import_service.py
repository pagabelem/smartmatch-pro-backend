...
class ImportService:
    ...
    async def import_jobs_csv(
        ...
        try:
            ...
            if jobs_to_create:
                await JobService.bulk_create(db, jobs_to_create)
    ...
    async def import_skills_csv(
        ...
        Colonnes optionnelles : category, display_name
        """
        record = Import(
            filename=filename,
            source=ImportSource.CSV.value,
        ...
        try:
            ...
            for _, row in df.iterrows():
                try:
                    raw_category = row.get("category") or row.get("sub_category")
                    category = _safe_enum(SkillCategory, raw_category, None)

                    skill_data = SkillCreate(
                        name=str(row["name"]),
                        display_name=str(row.get("display_name", "")) or None,
                        category=category,
                        skill_type=_safe_enum(SkillType, row.get("skill_type"), SkillType.HARD),
                    )
                    await skill_service.create_skill(db, skill_data)
                    imported += 1
                except Exception:
                    failed += 1
    ...
    async def get_history(
        ...
        query = select(Import)
        if import_type:
            query = query.where(Import.import_type == import_type)
        query = query.order_by(desc(Import.created_at))
        return await paginate_query(db, query, params)

    # ── GET BY ID ──────────────────────────────────────────────────────────────
    @staticmethod
    async def get_by_id(db: AsyncSession, import_id: int) -> Import:
