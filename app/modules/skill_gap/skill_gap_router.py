...
async def analyze_for_job(
    ...
    try:
        result = await SkillGapService.analyze_for_job(db, user_id, job_id)
...
async def analyze_global(
    ...
    try:
        result = await SkillGapService.analyze_global(db, user_id)
    except ValueError as e:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=str(e),
...
async def get_skill_gap(
    ...
    result = await SkillGapService.get_by_job(db, user_id, job_id)
...
async def get_history(
    ...
    page = await SkillGapService.get_history(db, user_id, params)
