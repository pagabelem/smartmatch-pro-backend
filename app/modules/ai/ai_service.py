...
class AIService:
    ...
    # ── CHATBOT ───────────────────────────────────────────────────────────────
    @staticmethod
    async def chat(db: AsyncSession, user_id: int, message: str, history: list) -> dict:
        ...
        if any(w in msg_lower for w in ["bonjour", "salut", "hello"]):
            reply = (
                "Bonjour ! Je suis votre assistant SmartMatch Pro. "
                "Je peux vous aider à trouver des offres, analyser "
                "votre profil ou générer une lettre de motivation. "
    ...
    async def generate_letter(
        db: AsyncSession, user_id: int, job_id: int, tone: str = "professional"
    ) -> dict:
        ...
        if not profile:
            raise ValueError(f"Profil introuvable pour l'utilisateur {user_id}.")
        if not job:
...
{name}"""
    ...
    @staticmethod
    async def generate_roadmap(db: AsyncSession, user_id: int, job_id: int) -> dict:
        job_result = await db.execute(select(Job).where(Job.id == job_id))
        job = job_result.scalars().first()
        if not job:
            raise ValueError(f"Offre {job_id} introuvable.")

        skill_gap = await SkillGapService.analyze_for_job(db, user_id, job_id)
    ...
    async def generate_interview(
        db: AsyncSession, user_id: int, job_id: int, num_questions: int = 5
    ) -> dict:
        job_result = await db.execute(select(Job).where(Job.id == job_id))
        job = job_result.scalars().first()
        if not job:
        ...
        question_bank = [
            {"question": f"Décrivez votre expérience avec {skills[0] if skills else 'la technologie principale'}.", "category": "Technique"},
            {"question": "Parlez-moi d'un projet dont vous êtes particulièrement fier(e).", "category": "Expérience"},
            {"question": "Comment gérez-vous les situations de pression ou les délais serrés ?", "category": "Comportemental"},
            {"question": f"Pourquoi souhaitez-vous rejoindre {job.company} ?", "category": "Motivation"},
            {"question": "Comment restez-vous à jour avec les nouvelles technologies de votre domaine ?", "category": "Veille technologique"},
            {"question": "Décrivez votre méthode de travail en équipe.", "category": "Soft skills"},
            {"question": f"Quelle est votre approche pour résoudre un problème complexe en {skills[1] if len(skills) > 1 else 'programmation'} ?", "category": "Technique"},
            {"question": "Où vous voyez-vous dans 3 ans ?", "category": "Projection"},
            {"question": "Avez-vous des questions concernant le poste ou l'entreprise ?", "category": "Final"},
            {"question": "Quelle est votre plus grande force et votre principal axe d'amélioration ?", "category": "Auto-évaluation"},
        ]

        selected = question_bank[:num_questions]
        questions = [
            {"question_number": i + 1, "question": q["question"], "category": q["category"]}
    ...
    # ── DÉTECTION DE FRAUDE ───────────────────────────────────────────────────
    @staticmethod
    async def check_fraud(db: AsyncSession, job_id: int) -> dict:
        job_result = await db.execute(select(Job).where(Job.id == job_id))
        job = job_result.scalars().first()
        if not job:
            raise ValueError(f"Offre {job_id} introuvable.")
        ...
        suspicious_words = ["urgent", "immédiat", "facile", "sans expérience",
